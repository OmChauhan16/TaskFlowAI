...
const QuickAddBar = () => {
    ...
    const handleParse = async () => {
        if (!input.trim()) { toast.error('Please enter a task description'); return; }
    ...
    const handleCreate = async () => {
        if (!parsedTask) { toast.error('Please parse the task first'); return; }
        if (!selectedProject) { toast.error('Please select a project'); return; }
        try {
            setCreatingTask(true);
            await createTask({
            ...
            toast.success('✅ Task created successfully!', { duration: 3000, position: 'top-center' });
            setTimeout(() => {
                setInput('');
                setShowPreview(false);
                setIsExpanded(false);
            ...
            console.error('Create failed:', error);
            setCreatingTask(false);
            toast.error(error.response?.data?.message || 'Failed to create task', { duration: 4000 });
        }
    };

    const handleCancel = () => { setInput(''); setShowPreview(false); clearParsedTask(); };
    const handleClose = () => { setIsExpanded(false); handleCancel(); };
    ...
    return (
        <>
            ...
            {!isExpanded && (
                <button
                    onClick={() => setIsExpanded(true)}
                    className="fixed bottom-6 right-6 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white p-4 rounded-full shadow-2xl transition-all duration-300 z-50"
                    title="Quick Add Task"
                >
                    <MessageSquare className="w-6 h-6" />
                    <span className="absolute -top-1 -right-1 bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
            ...
            {isExpanded && (
                <div className="
                    fixed z-50 bg-white shadow-2xl border border-gray-200 overflow-hidden animate-slideUp
                    /* Mobile: full width pinned to bottom, no border-radius on bottom edges */
                    bottom-0 left-0 right-0 rounded-t-2xl
                    /* sm+: floating card bottom-right */
                    sm:bottom-6 sm:left-auto sm:right-6 sm:w-96 sm:rounded-2xl
                ">
                    {/* Header */}
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-3 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        ...
                        <button onClick={handleClose} className="text-white hover:bg-white/20 p-1 rounded transition">
                            <X className="w-4 h-4" />
                        </button>
                    </div>

                    {/* Scrollable content area — cap height on mobile so it doesn't cover whole screen */}
                    <div className="p-4 overflow-y-auto max-h-[70vh] sm:max-h-none">
                        {!showPreview ? (
                            <div>
                                <p className="text-sm text-gray-600 mb-3">Describe your task in plain English</p>
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="relative">
                                        ...
                                        <select
                                            value={selectedProject}
                                            onChange={(e) => setSelectedProject(e.target.value)}
                                            className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none appearance-none bg-white text-sm"
                                            disabled={loadingProjects}
                                        >
                                            <option value="">Select a project</option>
                                            {projects.map((project) => (
                                    ...
                                    {projects.length === 0 && !loadingProjects && (
                                        <p className="text-xs text-orange-600 mt-1">⚠️ No projects found. Create a project first.</p>
                                    )}
                                </div>
                                ...
                                <div className="relative">
                                    <textarea
                                        ...
                                        placeholder="E.g., Remind me to review the marketing deck by Friday"
                                        className="w-full px-4 py-3 pr-12 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none resize-none text-sm"
                                        rows="3"
                                        disabled={parseLoading || !selectedProject}
                                        autoFocus={selectedProject !== ''}
                                    />
                                ...
                                <p className="text-xs text-gray-500 mt-2">Press Ctrl+Enter to parse</p>
                                    ...
                                    <div className="space-y-1">
                                        {[
                                            ...
                                            <button
                                                ...
                                                onClick={() => setInput(example)}
                                                disabled={!selectedProject}
                                                className="text-xs text-purple-600 hover:text-purple-700 hover:underline block disabled:text-gray-400 disabled:cursor-not-allowed text-left"
                                            >
                                                "{example}"
                                            </button>
                                        ))}
                            ...
                            parsedTask && (
                                <div className="space-y-3">
                                    ...
                                    {/* Task Preview */}
                                    <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                                        <p className="font-semibold text-gray-900 mb-1 text-sm">{parsedTask.title}</p>
                                        {parsedTask.description && (
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {parsedTask.priority && (
                                            <div className="bg-white border rounded-lg p-2">
                                                <p className="text-xs text-gray-500">Priority</p>
                                                <p className="font-semibold text-sm capitalize">{parsedTask.priority}</p>
                                            </div>
                                        )}
                                        {parsedTask.dueDate && (
                                            <div className="bg-white border rounded-lg p-2">
                                                <p className="text-xs text-gray-500">Due Date</p>
                                                <p className="font-semibold text-sm">{new Date(parsedTask.dueDate).toLocaleDateString()}</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        <button
                                            onClick={handleCreate}
                                            disabled={creatingTask}
                                            className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-green-400 disabled:cursor-not-allowed text-white px-4 py-2.5 rounded-lg font-medium transition flex items-center justify-center gap-2 text-sm"
                                        >
                                            {creatingTask ? (
                                        ...
                                        <button
                                            onClick={handleCancel}
                                            disabled={creatingTask}
                                            className="px-4 py-2.5 border border-gray-300 rounded-lg hover:bg-gray-50 transition disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                                        >
                                            Edit
                                        </button>
                                    </div>
